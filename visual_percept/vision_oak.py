...
def affiner_depth(depth_raw, disp_raw, frame_gray):
    if USE_WLS and _wls_filter is not None:
        ...
        ratio         = np.where(mask_both,
                                 disp_raw.astype(np.float32) / np.maximum(filtered_disp, 1), 1.0)
...
def thread_acquisition_webcam(frame_queue, stop_event):
    ...
    print("[WEBCAM] 640x480 → clipping centré 640x400")
...
# Couleurs traits
COULEUR_TRAIT_NET   = (0, 220, 255)   # jaune vif  — traits forts
COULEUR_TRAIT_FAIBLE = (180, 100, 255) # violet     — traits faibles

# Seuil de largeur LSD qui sépare net/faible
SEUIL_LARGEUR_NET = 1.5


def _seuil_longueur(depth_map, longueur_min_px):
    """Calcule le seuil de longueur minimum ajusté à la profondeur."""
    if depth_map is None:
        return float(longueur_min_px)
    h, w = depth_map.shape
    dy = int(h * 0.125)
    dx = int(w * 0.125)
    cy, cx = h // 2, w // 2
    ...
    if valides.size == 0:
        return float(longueur_min_px)
    dist_mm = float(np.median(valides))
    facteur = max(0.3, min(1.0, 500.0 / max(dist_mm, 1.0)))
    return max(float(longueur_min_px), longueur_min_px / facteur)


def detecter_traits(gray, depth_map=None, longueur_min_px=4):
    """
    Détecte les segments linéaires via LSD sur image CLAHE.
    Deux passes sur la même image :
      - Passe 1 (LSD_REFINE_STD)  → traits nets et forts
      - Passe 2 (LSD_REFINE_NONE) → traits faibles supplémentaires

    Chaque segment est qualifié :
      "net"   → largeur LSD >= SEUIL_LARGEUR_NET
      "faible"→ largeur LSD <  SEUIL_LARGEUR_NET
    ...
    Retourne une liste de segments :
    {
        ...
        "longueur"    : float px
        "angle"       : float degrés
        "largeur"     : float estimée LSD
        "qualite"     : "net" | "faible"
        "couleur"     : BGR tuple
    }
    """
    seuil_px = _seuil_longueur(depth_map, longueur_min_px)

    # Léger flou avant LSD — fusionne micro-bruit sans tuer les vrais bords
    # Noyau 3×3 : suffisant pour supprimer le sel-et-poivre, préserve les traits courts
    img = cv2.GaussianBlur(gray, (3, 3), 0)

    segments  = []
    vus       = set()   # évite les doublons entre les deux passes

    def _extraire(lines, widths):
        for i, line in enumerate(lines):
            x1, y1, x2, y2 = line[0]
            dx_ = x2 - x1
            dy_ = y2 - y1
            lon = math.sqrt(dx_*dx_ + dy_*dy_)
            if lon < seuil_px:
                continue
            # Clé de déduplication (arrondi à 2px)
            cle = (round(x1/2)*2, round(y1/2)*2, round(x2/2)*2, round(y2/2)*2)
            if cle in vus:
                continue
            vus.add(cle)
            larg    = float(widths[i][0]) if widths is not None else 1.0
            qualite = "net" if larg >= SEUIL_LARGEUR_NET else "faible"
            segments.append({
                "p1"     : (int(x1), int(y1)),
                "p2"     : (int(x2), int(y2)),
                "longueur": round(lon, 1),
                "angle"  : round(math.degrees(math.atan2(dy_, dx_)), 1),
                "largeur": round(larg, 2),
                "qualite": qualite,
                "couleur": COULEUR_TRAIT_NET if qualite == "net" else COULEUR_TRAIT_FAIBLE,
            })

    # Passe 1 — raffinement standard → traits nets
    lsd1 = cv2.createLineSegmentDetector(cv2.LSD_REFINE_STD)
    l1, w1, _, _ = lsd1.detect(img)
    if l1 is not None:
        _extraire(l1, w1)

    # Passe 2 — sans raffinement → attrape les traits faibles manqués
    lsd2 = cv2.createLineSegmentDetector(cv2.LSD_REFINE_NONE)
    l2, w2, _, _ = lsd2.detect(img)
    if l2 is not None:
        _extraire(l2, w2)
...
def _extraire_formes(gray, aire_parente, precision, profondeur, lum_globale,
    ...
    # Noyau gaussien adapté au facteur de downsampling (impair, min 3)
    k_gauss = max(3, (div // 2) | 1)

    sw, sh  = max(1, w // div), max(1, h // div)
    small_raw = cv2.resize(gray, (sw, sh), interpolation=cv2.INTER_AREA)

    # Flou avant morpho — lisse les artefacts de downsampling sans détruire les contours
    small   = cv2.GaussianBlur(small_raw, (k_gauss, k_gauss), 0)

    morphed_open  = cv2.morphologyEx(small, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
    morphed       = cv2.morphologyEx(morphed_open, cv2.MORPH_CLOSE,
                                     cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)))

    # Canny directement sur small — pas d'upsample
    edges   = cv2.Canny(morphed, canny_low, canny_high)

    # Étapes debug conservées pour affichage éventuel
    if debug:
        _debug_etapes["gray"]    = (gray,      f"Gray+CLAHE  lum:{np.mean(gray):.0f}")
        _debug_etapes["small"]   = (small_raw,  f"Downsample  div:{div}  {sw}x{sh}")
        _debug_etapes["blur"]    = (small,      f"GaussBlur   k:{k_gauss}x{k_gauss}")
        _debug_etapes["morpho"]  = (morphed,    f"Morpho OPEN+CLOSE")
        _debug_etapes["edges"]   = (edges,      f"Canny  lo:{canny_low} hi:{canny_high}")

    contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours     = sorted(contours, key=cv2.contourArea, reverse=True)

    # Seuil d'aire adapté à la résolution downsamplée
    seuil_aire_small = seuil_aire / (div * div)

    objets         = []
    masque_utilise = np.zeros((sh, sw), dtype=np.uint8)

    for cnt in contours:
        if cv2.contourArea(cnt) < seuil_aire_small:
            continue

        mask_test = np.zeros((sh, sw), dtype=np.uint8)
        ...
        if pata:
            # Remonter l'ellipse en coordonnées scène
            (ex, ey), axes, angle = ellipse
            ellipse_scene = ((ex * div, ey * div), (axes[0] * div, axes[1] * div), angle)
            sommets = ellipse_en_points(ellipse_scene, n=24)
            label   = "patatoïde"
        else:
            hull    = cv2.convexHull(cnt)
            approx  = simplifier_convexe(hull, n_max=8)
            # Rescaler les coordonnées vers la résolution scène
            sommets = [(int(pt[0][0] * div), int(pt[0][1] * div)) for pt in approx]
            label   = classifier_forme(sommets)

        if len(sommets) < 3:
            continue

        # Photométrie sur l'image grise originale (pleine résolution)
        ...
        aire_cnt  = int(cv2.contourArea(cnt) * div * div)   # aire rescalée
...
# Dictionnaire partagé pour les étapes debug (niveau 0 uniquement)
_debug_etapes = {}
...
def vision_primitive_complete(frame, depth_map=None, z_cible=None, precision=0.5):
    ...
    # Niveau 0 — scène complète (avec capture debug)
    _debug_etapes.clear()
    objets = _extraire_formes(gray, aire_scene, precision, 0, lum_globale, debug=True)

    # Niveau 1 — ROI de chaque forme (si PROFONDEUR_MAX >= 1)
    for obj in objets:
        ...
        enfants = _extraire_formes(roi_gray, aire_roi, precision, 1, lum_globale, debug=False)

        # Recalage coords enfants → repère scène
    ...
    # Pipeline parallèle — traits linéaires indépendants des formes
    traits = detecter_traits(gray, depth_map=depth_map)

    return objets, traits, {"lum_globale": round(lum_globale, 2)}


# ─────────────────────────────────────────────────────────────
#  DESSIN — récursif pour supporter N niveaux
# ─────────────────────────────────────────────────────────────
def afficher_debug(frame, objets):
    """
    Ouvre une fenêtre par étape du pipeline de traitement.
    Appelée en mode analyse (ESPACE). Chaque vignette affiche
    l'étape et ses paramètres clés.
    """
    fonte = cv2.FONT_HERSHEY_SIMPLEX

    def _annoter(img_gray, titre):
        """Convertit en BGR, upscale à 640x400, ajoute le titre."""
        if len(img_gray.shape) == 2:
            vis = cv2.cvtColor(img_gray, cv2.COLOR_GRAY2BGR)
        else:
            vis = img_gray.copy()
        # Upscale pour lisibilité
        vis = cv2.resize(vis, (640, 400), interpolation=cv2.INTER_NEAREST)
        # Fond semi-transparent pour le texte
        overlay = vis.copy()
        cv2.rectangle(overlay, (0, 0), (640, 22), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.6, vis, 0.4, 0, vis)
        cv2.putText(vis, titre, (6, 15), fonte, 0.45, (0, 255, 180), 1)
        return vis
    ...
    for titre_fenetre, cle in etapes:
        if cle not in _debug_etapes:
            continue
        img, params = _debug_etapes[cle]
        label_complet = f"{titre_fenetre}  |  {params}"
        vis = _annoter(img, label_complet)
        cv2.imshow(titre_fenetre, vis)

    # Fenêtre 6 : overlay résultat
    overlay_frame = dessiner_overlay(frame, objets)
    n_enfants = sum(len(o["enfants"]) for o in objets)
    label_overlay = f"6. Résultat  |  {len(objets)} forme(s)  {n_enfants} sous-forme(s)"
    cv2.rectangle(overlay_frame, (0, 0), (640, 22), (0, 0, 0), -1)
    cv2.putText(overlay_frame, label_overlay, (6, 15), fonte, 0.45, (0, 255, 180), 1)
    cv2.imshow("6. Résultat", overlay_frame)


def fermer_debug():
    """Ferme toutes les fenêtres debug."""
    for titre, _ in [("1. Gray+CLAHE", ""), ("2. Downsample", ""),
                     ("3. GaussBlur", ""), ("4. Morpho", ""),
                     ("5. Canny", ""), ("6. Résultat", "")]:
        try:
...
def _dessiner_objet(canvas, obj):
    """Dessine un objet et tous ses enfants récursivement."""
    pts      = np.array(obj["points"], dtype=np.int32)
    couleur  = obj["couleur"]
    epaisseur = max(1, 2 - obj["profondeur"])   # 2px niveau 0, 1px niveau 1+

    cv2.drawContours(canvas, [pts], -1, couleur, epaisseur)

    x, y, w, h = obj["bbox"]
    cv2.rectangle(canvas, (x, y), (x + w, y + h), couleur, 1)

    # Label : taille de fonte décroissante par niveau
    fonte = max(0.3, 0.5 - obj["profondeur"] * 0.1)
    cv2.putText(canvas, obj["label"], (x, max(y - 4, 10)),
                cv2.FONT_HERSHEY_SIMPLEX, fonte, couleur, 1)

    for enfant in obj.get("enfants", []):
        _dessiner_objet(canvas, enfant)


def dessiner_overlay(frame, objets, traits=None):
    resultat = frame.copy()

    # Traits linéaires — couleur selon qualité (net=jaune, faible=violet)
    if traits:
        for t in traits:
            epaisseur = 2 if t["qualite"] == "net" else 1
            cv2.line(resultat, t["p1"], t["p2"], t["couleur"], epaisseur, cv2.LINE_AA)

    # Formes géométriques et patatorides
    for obj in objets:
        _dessiner_objet(resultat, obj)
...
if __name__ == "__main__":
    ...
    print("ESPACE (à nouveau) : retour temps réel")
    print("Q : quitter\n")

    while not stop_event.is_set():
        # Dépile la dernière frame disponible
        try:
        ...
        if mode_analyse and resultats_analyse is not None:
            ...
            cv2.putText(affichage, "ESPACE : analyser", (8, 18),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
        else:
            continue

        cv2.imshow("Vision Primitive", affichage)
        ...
        if key == ord('q'):
            ...
            if not mode_analyse and derniere_frame is not None:
                ...
                z_cible = (distance_mediane_centrale(derniere_depth)
                           if derniere_depth is not None else 0.0)
                objets, traits, scene = vision_primitive_complete(
                    derniere_frame,
                    depth_map=derniere_depth,
                    z_cible=z_cible if z_cible > 0 else None,
                    precision=PRECISION
                )
                resultats_analyse = dessiner_overlay(derniere_frame, objets, traits)
                afficher_debug(derniere_frame, objets)
                mode_analyse      = True
                n_enfants = sum(len(o["enfants"]) for o in objets)
                print(f"{len(objets)} forme(s) — {n_enfants} sous-forme(s) — {len(traits)} trait(s)")
            else:
                mode_analyse      = False
                resultats_analyse = None
                fermer_debug()
