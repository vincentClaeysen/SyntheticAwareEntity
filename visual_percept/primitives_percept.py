...
ESPACE : analyser la frame courante et figer l'affichage
...
# ─────────────────────────────────────────────────────────────
#  COULEURS PAR TYPE D'OBJET (pour l'overlay)
# ─────────────────────────────────────────────────────────────
COULEURS_PAR_LABEL = {
    ...
    "default":    (0, 255, 255)     # jaune par défaut
}
...
# ─────────────────────────────────────────────────────────────
#  FONCTIONS VISION (extraites du code original)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
#  THREAD TRAITEMENT (modifié pour l'analyse sur demande)
# ─────────────────────────────────────────────────────────────
def thread_traitement(frame_queue: queue.Queue,
                      analyse_queue: queue.Queue,
                      stop_event: threading.Event):
    """
    - Lit en continu les frames de la file d'acquisition
    - Quand une analyse est demandée (analyse_queue non vide), 
      exécute la vision sur la dernière frame reçue
    - Renvoie le résultat dans analyse_queue
    """
    derniere_frame = None
    derniere_depth = None
    dernier_ts = None
...
if __name__ == "__main__":
    ...
    analyse_queue = queue.Queue(maxsize=1)  # une analyse à la fois
    stop_event = threading.Event()
    ...
    mode_analyse = False  # False = flux temps réel, True = affichage figé
    frame_affichee = None
    ...
    print("ESPACE : analyser la frame courante et figer l'affichage")
    ...
    while not stop_event.is_set():
        if mode_analyse:
            # Mode figé : afficher la dernière analyse
            if frame_affichee is not None:
                affichage = frame_affichee.copy()
                
                # Fonction récursive pour dessiner un objet et ses enfants
                def dessiner_objet(canvas, obj):
                    pts = np.array(obj["points"], dtype=np.int32)
                    couleur = obj["couleur"]
                    cv2.drawContours(canvas, [pts], -1, couleur, 2)
                    x, y, w, h = obj["bbox"]
                    cv2.rectangle(canvas, (x, y), (x+w, y+h), couleur, 1)
                    cv2.putText(canvas, obj["label"], (x, y-5),
                ...
                # Informations
                z_str = f"z:{z_cible:.0f}mm" if z_cible > 0 else "z:N/A"
                hud = f"ANALYSE FIGEE | {scene_stats['lum_globale']:.0f} | {z_str} | ESPACE pour reprendre"
