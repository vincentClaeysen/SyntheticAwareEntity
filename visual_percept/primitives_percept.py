...
def _extraire_formes(gray, aire_parente, precision, profondeur, lum_globale):
    ...
    for cnt in contours:
        ...
        objets.append({
            ...
            "points"     : sommets,
            "centre"     : (int(x + wb / 2), int(y + hb / 2)),   # repère local ROI
...
def vision_primitive_complete(frame, depth_map=None, z_cible=None, precision=0.5):
    ...
    # Niveau 0 — scène complète
    objets = _extraire_formes(gray, aire_scene, precision, 0, lum_globale)
    ...
    for obj in objets:
        ...
        enfants = _extraire_formes(roi_gray, aire_roi, precision, 1, lum_globale)
...
def dessiner_overlay(frame, objets):
    resultat = frame.copy()
    for obj in objets:
    ...
    while not stop_event.is_set():
        ...
        if key == ord('q'):
            ...
            if not mode_analyse and derniere_frame is not None:
                ...
                resultats_analyse = dessiner_overlay(derniere_frame, objets)
                mode_analyse      = True
                n_enfants = sum(len(o["enfants"]) for o in objets)
                print(f"{len(objets)} forme(s) — {n_enfants} sous-forme(s)")
            else:
                mode_analyse      = False
                resultats_analyse = None
